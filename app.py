...
@app.route("/bus/<operator>/<line>/timetable", methods=["POST"])
def timetable(operator, line):
    session_data = libbusstop.load_data()
    url = request.form["url"]
    session_data.timetable(url)
    timetable_data = session_data.timetable_data
    stops_num = len(timetable_data["stops"])
    session_data.save_data()
    return render_template("timetable.html", timetable_data=timetable_data, stops_num=stops_num)
